...
class Config {
    ...
    static let urlBase = "https://app.antapaccay.com.pe/hsecwebp/whsec_Service/api"
    
    static func getAllMaestro(_ shouldBlock: Bool, _ checkOnline: Bool) {
        if shouldBlock {
        ...
        if checkOnline {
            ...
            Rest.getDataGeneral(Routes.forMaestroALL(), false, success: {(resultValue:Any?,data:Data?) in
                ...
                let maestros = arrayMaestros.Data
                ...
                for i in 0..<maestros.count {
                    ...
                    var descripcion = maestros[i].Descripcion ?? ""
                    ...
                    case "SUPE":
                        descripcion = descripcion.trimmingCharacters(in: CharacterSet.init(charactersIn: " -="))
                        let splits = codTipo.components(separatedBy: ".")
                        if splits.count == 1 {
                            newMaestroCodTipo[codigo]!.append(codTipo)
                            newMaestroDescripcion[codigo]!.append(descripcion)
    ...
    /*static func getAllMaestro(_ shouldBlock: Bool) {
        Utils.maestroCodTipo = globals.dictionary(forKey: "HSECMaestroCodTipo") as? [String : [String]] ?? [:]
        Utils.maestroDescripcion = globals.dictionary(forKey: "HSECMaestroDescripcion") as? [String : [String]] ?? [:]
        if shouldBlock {
            Utils.bloquearPantalla()
